...
describe("Feed", () => {
  ...
  it('show appropriate "Your Feed" from fixture', () => {
    ...
    cy.get('[data-testid=article-preview]').eq(0).find('h1')
        .should('contain', 'Cypress Workshop Berlin');
    cy.get('[data-testid=article-preview]').eq(0).find('p')
        .should('contain', 'Ein Workshop über E2E testinmg');

    cy.get('[data-testid=article-preview]').eq(1).find('h1')
        .should('contain', 'Effizientes e2e');
    cy.get('[data-testid=article-preview]').eq(1).find('p')
